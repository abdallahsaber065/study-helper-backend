...
async def generate_quiz(
    ...
    try:
        ...
        file_metadata = file_result.scalar_one_or_none()
...
async def list_quizzes(
    ...
    status_filter: Optional[List[str]] = Query(
        None, description="Filter by quiz status"
    ),
    difficulty_level: Optional[List[str]] = Query(
    ...
    min_quality_score: Optional[float] = Query(
        None, ge=0.0, le=5.0, description="Minimum quality score"
    ),
    has_user_rating: Optional[bool] = Query(
        None, description="Filter by user rating presence"
    ),
    topic: Optional[str] = Query(None, description="Filter by topic"),
    learning_objective: Optional[str] = Query(
        None, description="Filter by learning objective"
    ),
    question_count_min: Optional[int] = Query(
        None, ge=1, description="Minimum question count"
    ),
    question_count_max: Optional[int] = Query(
        None, le=50, description="Maximum question count"
    ),
    search_query: Optional[str] = Query(
        None, description="Search in title, description, topics"
    ),
    # Pagination
    ...
    try:
        ...
        # Build filters
        filters = QuizFilters(
            status=[s for s in status_filter] if status_filter else None,
            difficulty_level=(
                [d for d in difficulty_level] if difficulty_level else None
            ),
        ...
        return await quiz_service.get_quizzes(current_user.id, filters)
...
async def get_quiz(
    ...
    shuffle_questions: Optional[bool] = Query(
        None, description="Override question shuffling"
    ),
    shuffle_answers: Optional[bool] = Query(
        None, description="Override answer shuffling"
    ),
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db_session),
):
    ...
    try:
        ...
        if include_questions:
            quiz_response = await quiz_service.get_quiz_with_questions(
                quiz_id=quiz_id,
                user_id=current_user.id,
            ...
            metrics = await quiz_service._calculate_quiz_metrics(quiz)
            quiz_response = QuizResponse(
                ...
                topics_covered=(
                    json.loads(quiz.topics_covered) if quiz.topics_covered else []
                ),
                learning_objectives=(
                    json.loads(quiz.learning_objectives)
...
async def update_quiz(
    ...
    try:
        ...
        quiz = await quiz_service.update_quiz(quiz_id, current_user.id, update_data)
...
async def delete_quiz(
    ...
    try:
        ...
        success = await quiz_service.delete_quiz(
            quiz_id, current_user.id, soft_delete=not permanent
        )
        ...
        logger.info(
            f"Deleted quiz {quiz_id} for user {current_user.id} (permanent: {permanent})"
        )
...
async def archive_quiz(
    ...
    try:
        ...
        success = await quiz_service.archive_quiz(quiz_id, current_user.id)
...
async def regenerate_quiz_questions(
    ...
    try:
        ...
        regeneration_config = {
            "ai_provider": (
                request.ai_provider.value if request.ai_provider else quiz.ai_provider
            ),
            "ai_model": request.ai_model or quiz.ai_model_used,
            "difficulty_level": (
                request.difficulty_level.value
                if request.difficulty_level
                else quiz.difficulty_level
            ),
...
async def get_quiz_analytics(
    ...
    try:
        ...
        analytics = await quiz_service.get_quiz_analytics(quiz_id, current_user.id)
...
async def start_quiz_attempt(
    ...
    try:
        ...
        return QuizAttemptResponse(
            ...
            topic_scores=(
                json.loads(attempt.topic_scores) if attempt.topic_scores else None
            ),
            difficulty_progression=(
                json.loads(attempt.difficulty_progression)
                if attempt.difficulty_progression
                else None
            ),
...
async def get_quiz_attempt(
    ...
    include_responses: bool = Query(
        False, description="Include individual question responses"
    ),
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db_session),
):
    ...
    try:
        ...
        if include_responses:
            ...
            for response_obj in attempt.responses:
                response_detail = QuestionResponseDetail(
                    ...
                    selected_answer_ids=(
                        json.loads(response_obj.selected_answer_ids)
                        if response_obj.selected_answer_ids
                        else None
                    ),
        ...
        logger.error(
            f"Failed to get quiz attempt {attempt_id}: {str(e)}", exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def list_quiz_attempts(
    ...
    try:
        ...
        return attempt_service.get_user_attempts(
            user_id=current_user.id, quiz_id=quiz_id, page=page, page_size=page_size
        )
